...
- **앱 내부(`apps/{app}/src/ui`)의 도메인 폴더끼리도 함부로 공유하지 않는다.** `ui/product`, `ui/portfolio`처럼 서로 다른 도메인 폴더가 구조적으로 비슷한 로직(예: 좋아요 토글 훅, API 뮤테이션 훅)을 필요로 하더라도, 그 로직이 도메인 지식(무엇에 대한 동작인지)을 갖고 있다면 각 도메인 폴더 안에 독립적으로 둔다 — `ui/` 루트나 별도 공용 폴더로 끌어올리지 않는다. 코드가 반복된다는 사실이 곧 "공통"임을 의미하지 않는다. 도메인 지식 없이도(어떤 도메인인지 몰라도) 이해되는 코드만 공용화 후보다. 자세한 이유는 [ADR-0002](../../../../docs/adr/0002-domain-coupled-code-stays-local.md) 참고.
